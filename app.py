...
if view_mode not in {"Bybit만 보기", "Bybit Swing PAPER만 보기"}:
    ...
    def _bot_state_get(key: str, default: str = "") -> str:
        if not OKX_BOT_DB.exists():
            return default
        try:
            with sqlite3.connect(OKX_BOT_DB) as conn:
    ...
    if OKX_BOT_CONFIG.exists():
        try:
            ...
            st.info(
                f"모드 {mode} · 격리 {bot_cfg.get('leverage', 5)}배 · "
                f"일일 진입 {'무제한' if int(bot_cfg.get('max_daily_entries', 0)) == 0 else str(bot_cfg.get('max_daily_entries')) + '회'} · "
        ...
        try:
            with sqlite3.connect(OKX_BOT_DB) as bot_conn:
                ...
                bot_events = bot_conn.execute(
                    "SELECT * FROM bot_events ORDER BY id DESC LIMIT 40"
                ).fetchall()
                trade_history_events = bot_conn.execute(
                    "SELECT * FROM bot_events WHERE COALESCE(trade_id,'')<>'' ORDER BY id DESC LIMIT 300"
                ).fetchall()
            ...
            pause_new = _bot_state_get("pause_new_entries", "0") == "1"
            shutdown_wait = _bot_state_get("shutdown_when_flat", "0") == "1"
            process_status = _bot_state_get("bot_process_status", "UNKNOWN")

            st.markdown("### 🛠️ 봇 제어")
            if shutdown_wait:
                st.warning(f"안전 종료 예약됨 · 신규 진입 중지 · 현재 포지션 {len(bot_positions)}개 관리 중")
            elif pause_new:
                st.warning(f"업데이트 대기 모드 · 신규 진입 중지 · 현재 포지션 {len(bot_positions)}개 관리 중")
            else:
                st.success("봇 실행 중 · 신규 진입 허용")
            st.caption(f"프로세스 상태: {process_status}")

            b1, b2, b3 = st.columns(3)
            if b1.button("⏸️ 신규 진입 중지", use_container_width=True, disabled=pause_new or shutdown_wait):
                _bot_state_set("pause_new_entries", "1")
                _bot_state_set("shutdown_when_flat", "0")
                st.rerun()
            if b2.button("▶️ 신규 진입 재개", use_container_width=True, disabled=(not pause_new) or shutdown_wait):
                _bot_state_set("pause_new_entries", "0")
                _bot_state_set("shutdown_when_flat", "0")
                st.rerun()
            if b3.button("⏹️ 봇 종료(안전)", use_container_width=True, disabled=shutdown_wait):
                _bot_state_set("pause_new_entries", "1")
                _bot_state_set("shutdown_when_flat", "1")
                st.rerun()
            st.caption("안전 종료는 신규 진입을 즉시 막고, 보유 포지션을 계속 관리한 뒤 포지션이 0이 되면 봇 프로세스를 종료합니다.")

            c1, c2, c3 = st.columns(3)
            entry_limit = int(bot_cfg.get("max_daily_entries", 0))
            entry_text = f"{int(today_stats['entries'] or 0)}회" if entry_limit == 0 else f"{int(today_stats['entries'] or 0)} / {entry_limit}"
            c1.metric("오늘 진입", entry_text)
            ...
            st.markdown("### 📒 OKX PAPER 매매기록")
            trade_events = list(trade_history_events)
            grouped = {}
            for e in reversed(trade_events):
                tid = e["trade_id"] or f"legacy-{e['symbol']}"
                grouped.setdefault(tid, []).append(e)
            if not grouped:
                st.info("v3.8.9부터 진입·순환추가·추가분 회수·TP·손절 기록이 거래별로 표시됩니다.")
            else:
                event_names = {
                    "ENTRY": "최초 진입", "REBOUND_ADD": "순환추가",
                    "CYCLE_REDUCE": "추가분 회수", "TP1": "TP1 익절", "TP2": "TP2 익절",
                    "STOP": "손절", "STOP_HALF": "1차 손절", "FINAL_STOP": "최종 손절",
                    "RECOVERY_EXIT": "회복 종료", "MANUAL_EXIT": "수동 종료",
                    "BE_EXIT": "본절 보호 종료", "FLAT_EXIT_75M": "정체 종료", "TIME_EXIT": "시간 종료"
                }
                for tid, events in list(reversed(list(grouped.items())))[:12]:
                    first = events[0]
                    total_pnl = sum(float(x["realized_pnl"] or 0) for x in events)
                    closed = any(x["event"] in {"TP2","STOP","FINAL_STOP","RECOVERY_EXIT","MANUAL_EXIT","BE_EXIT","FLAT_EXIT_75M","TIME_EXIT"} for x in events)
                    status = "종료" if closed else "진행 중"
                    with st.expander(f"{first['symbol']} · {status} · 누적 {total_pnl:+.2f} USDT", expanded=False):
                        for e in events:
                            try:
                                d = json.loads(e["details"] or "{}")
                            except Exception:
                                d = {}
                            label = event_names.get(e["event"], e["event"])
                            if e["event"] == "ENTRY":
                                st.write(f"① {label} · 진입가 {float(e['price']):.10g} · 증거금 {float(d.get('margin_usdt',0)):.2f} USDT · 수량 {float(e['qty']):.8g}")
                            elif e["event"] == "REBOUND_ADD":
                                st.write(f"② {label} · 추가가 {float(e['price']):.10g} · +{float(d.get('add_margin_usdt',0)):.2f} USDT · 평단 {float(d.get('previous_avg',0)):.10g} → {float(d.get('new_avg',0)):.10g}")
                            elif e["event"] == "CYCLE_REDUCE":
                                st.write(f"③ {label} · 회수가 {float(e['price']):.10g} · 추가수량 정리 · 손익 {float(e['realized_pnl'] or 0):+.2f} USDT · 평단 {float(d.get('restored_base_avg',0)):.10g} 복귀")
                            elif e["event"] in {"TP1","TP2","STOP","BE_EXIT","FLAT_EXIT_75M","TIME_EXIT"}:
                                st.write(f"④ {label} · 청산가 {float(e['price']):.10g} · 이번 손익 {float(e['realized_pnl'] or 0):+.2f} USDT · 거래 누적 {float(d.get('trade_total_realized_pnl', total_pnl)):+.2f} USDT")
                        if any(e["event"] == "REBOUND_ADD" for e in events) and not any(e["event"] == "CYCLE_REDUCE" for e in events):
                            st.warning("순환추가분 회수 기록 없음: 추가 물량을 보유한 채 TP·손절됐는지 확인하세요.")
        ...
        try:
            ...
            for row in positions:
                with st.container(border=True):
                    ...
                    base = float(row['base_entry_price'] or avg) if 'base_entry_price' in row.keys() else avg
                    last = float(row['last_price'] or avg)
                    qty = float(row['total_qty'] or 0)
                    tp1 = base * (1 + float(bs_cfg.get('tp1_pct',1.5))/100)
                    tp2 = base * (1 + float(bs_cfg.get('tp2_pct',3.0))/100)
                    stage1 = base * (1 - float(bs_cfg.get('stage1_stop_pct',1.5))/100)
                    final_stop = base * (1 - float(bs_cfg.get('final_stop_pct',3.0))/100)
                    be = base * (1 - float(bs_cfg.get('breakeven_stop_pct',0.05))/100)
                    fee_rate = float(bs_cfg.get('taker_fee_rate',0.00055))
                    gross = (last-avg)*qty
                    est_fees = (avg*qty + last*qty)*fee_rate
                    net = gross-est_fees
                    ...
                    held_min=max(0,int((datetime.now(timezone.utc)-opened).total_seconds()/60))
                    max_min=int(float(bs_cfg.get('max_hold_hours',3))*60)
                    st.markdown(f"### {symbol} · {row['strategy'] or 'P'}형 · {row['note'] or '관리 중'}")
                    st.caption(f"진입시간(KST) {_kst_time(row['opened_at'])} · 보유 {held_min//60}시간 {held_min%60}분 · 시간종료까지 약 {max(0,max_min-held_min)}분")
                    a,b,c=st.columns(3)
                    a.metric("최초 진입가",format(base,'.10g'))
                    b.metric("현재 평단",format(avg,'.10g'), delta=(None if abs(avg-base)<1e-15 else f"{(avg/base-1)*100:+.2f}% 변경"))
                    c.metric("현재가",format(last,'.10g'), delta=f"{(last/base-1)*100:+.2f}%")
                    t1,t2,t3=st.columns(3)
                    t1.metric("TP1",format(tp1,'.10g'),delta=f"+{float(bs_cfg.get('tp1_pct',1.5)):.2f}%")
                    ...
                    if pending_symbol==symbol:
                        st.warning("수동 종료 요청 처리 대기 중입니다.")
            if not positions:
                st.info("현재 Bybit Swing PAPER 보유 포지션이 없습니다.")

            try:
                with sqlite3.connect(BYBIT_SWING_DB) as review_conn:
                    review_conn.row_factory=sqlite3.Row
                    reviews=review_conn.execute("SELECT * FROM stop_reviews ORDER BY id DESC LIMIT 12").fetchall()
                if reviews:
                    with st.expander("🔎 종료 후 3시간 추적",expanded=False):
                        for r in reviews:
                            hit_tp1 = ''
                            if r['entry_price'] and r['price_180m']:
                                hit_tp1 = ' · 3시간 가격 기준 TP1 이상' if (float(r['price_180m'])/float(r['entry_price'])-1)*100 >= float(bs_cfg.get('tp1_pct',1.5)) else ''
                            st.write(f"{r['symbol']} · {r['stop_event']} · 종료 {_kst_time(r['stop_ts'])} · 1h {('-' if r['pct_60m'] is None else f'{float(r["pct_60m"]):+.2f}%')} · 2h {('-' if r['pct_120m'] is None else f'{float(r["pct_120m"]):+.2f}%')} · 3h {('-' if r['pct_180m'] is None else f'{float(r["pct_180m"]):+.2f}%')}{hit_tp1}")
            except Exception:
                pass
            ...
            names={"ENTRY":"최초 진입","REBOUND_ADD":"순환추가","CYCLE_REDUCE":"추가분 회수","TP1":"TP1 익절","TP2":"TP2 익절","STOP":"손절","STOP_HALF":"1차 손절","FINAL_STOP":"최종 손절","RECOVERY_EXIT":"회복 종료","MANUAL_EXIT":"수동 종료","BE_EXIT":"본절 보호 종료","FLAT_EXIT_75M":"정체 종료","TIME_EXIT":"시간 종료"}
